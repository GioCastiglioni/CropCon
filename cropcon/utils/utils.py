...
class ConsistentTemporalTransform(torch.nn.Module):
    def __init__(self, h_w=128, degrees=30, view: int = 1):
        super().__init__()
        self.degrees = degrees
        self.h_w = h_w
        ...
        self.transforms = v2.Compose([
            v2.RandomResizedCrop(size=(h_w, h_w), scale=(0.2, 1.0), antialias=True),
            v2.RandomHorizontalFlip(p=0.5),
            v2.RandomVerticalFlip(p=0.5),
            v2.RandomApply([v2.GaussianBlur(kernel_size=11, sigma=(0.1, 2.0))], p=1.0 if view == 1 else 0.1)
            ])
    ...
    def forward(self, sample):
        img_tv = tv_tensors.Image(sample["image"])
        
        img_transformed = self.transforms(img_tv)

        img_tensor = torch.as_tensor(img_transformed)

        angle = torch.empty(1).uniform_(-self.degrees, self.degrees).item()
        img_rotated = self.rotate_with_reflection_padding(img_tensor, angle, is_mask=False)

        img_final = self.add_gaussian_noise(img_rotated)

        return {"image": img_final}

    def rotate_with_reflection_padding(self, img, angle, is_mask=False):
        """
        img: Tensor de forma (T, C, H, W)
    ...
    def add_gaussian_noise(self, img, std_ratio=0.02):
        ...
        img_min = img.amin(dim=dims, keepdim=True)
        img_max = img.amax(dim=dims, keepdim=True)
        dynamic_range = img_max - img_min + 1e-8
        std = std_ratio * dynamic_range
        noise = torch.randn_like(img) * std
        noisy_img = torch.max(img_min, torch.min(img + noise, img_max))
        return noisy_img