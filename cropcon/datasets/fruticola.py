...
class CropsChileFruticola(RawGeoFMDataset):
    def __init__(
        self,
        split: str,
        dataset_name: str,
        ...
        # split by region if specified
        if region is not None:
            if "region" not in self.meta_patch.columns:
                raise KeyError("Column 'region' not found in metadata.")
            self.meta_patch = self.meta_patch[self.meta_patch["region"] == region].copy()
    ...
    def __getitem__(self, i: int) -> Dict[str, Union[torch.Tensor, Dict[str, torch.Tensor]]]:
        ...
        for modality in self.modalities:
            ...
            if array.ndim == 4:
                ...
                idx = torch.linspace(0, array.shape[0] - 1, self.multi_temporal, dtype=torch.long)
            ...
            if modality == "S2":
                ...
                if dates.numel() > 0:
                    idx = torch.linspace(0, dates.shape[0] - 1, self.multi_temporal, dtype=torch.long)
        ...
        return {
            "image": {"optical": data["optical"]},
            "target": target.to(torch.int64),
            "metadata": metadata,
        }

    @staticmethod
    def download(root_path: str = None, *args, **kwargs):
        pass